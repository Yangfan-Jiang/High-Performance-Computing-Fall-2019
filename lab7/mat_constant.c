...
__constant__ float cuda_b[N];
//__constant__ float cuda_c[N];

__global__ void MatVecMultiply(float a[][N], float y[N]) {
...
{   
    ...
    start = clock();
    //cudaMemcpyToSymbol(cuda_A, A, sizeof(float)*N*N, cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(cuda_b, b, sizeof(float)*N);
	...
	dim3 numBlocks(4, 4);
	MatVecMultiply << <numBlocks, threadsPerBlock>> >(device_a, device_c);
	//finish = clock();
    //cudaMemcpyFromSymbol(C2, cuda_c, sizeof(float)* N, cudaMemcpyDeviceToHost);
